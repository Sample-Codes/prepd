...
class WF_OrderImpExpCsv_Order_Import extends WP_Importer {
	...
	public function import() {
		...
		$this->log->add( 'hf-order-csv-import', '---' );
		$this->log->add( 'hf-order-csv-import', __( 'Processing orders.', 'wf_order_import_export' ) );
                $merging = 1;
                $record_offset = 0;
		...
		$this->log->add( 'hf-order-csv-import', __( 'Finished processing Orders.', 'wf_order_import_export' ) );
	...
	public function import_start( $file, $mapping, $start_pos, $end_pos, $eval_field ) {
		...
		$this->log->add( 'hf-order-csv-import', '---[ New Import ] PHP Memory: ' . $memory . ', WP Memory: ' . $wp_memory );
		$this->log->add( 'hf-order-csv-import', __( 'Parsing order CSV.', 'wf_order_import_export' ) );
		...
		$this->log->add( 'hf-order-csv-import', __( 'Finished parsing order CSV.', 'wf_order_import_export' ) );
	...
	private function process_orders($post) {
		...
		//$dry_run = isset( $_POST['dry_run'] ) && $_POST['dry_run'] ? true : false;
                ...
                if (is_wp_error($user_id)) {
                   $this->log->add('hf-order-csv-import', sprintf(__($user_id->get_error_message(), 'wf_order_import_export')));
                   ...
                   $this->log->add('hf-order-csv-import', sprintf(__('An error occurred with the customer information provided.', 'wf_order_import_export')));
		...
		$this->log->add('hf-order-csv-import', __( 'Processing orders.', 'wf_order_import_export' ) );
            ...
            if (!$merging && $is_order_exist) {
                    ...
                    $this->log->add('hf-order-csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'wf_order_import_export'), esc_html($order_data['post_title'])), true);
                    unset($post);
                    return;
                }
            else{
                if($is_order_exist) { 
					...
					$this->log->add('hf-order-csv-import', sprintf( __( '> Error inserting %s: %s', 'wf_order_import_export'), $post['order_number'], $order_id->get_error_message() ), true );
                    }
			...
			if ( ! empty( $post['order_number_formatted'] ) ) {
				...
				$this->processed_posts[ $post['order_number_formatted'] ] = $post['order_number_formatted'];
                        ...
                        $this->log->add('hf-order-csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'wf_order_import_export'), esc_html($order_data['post_title'])), true);
			$this->imported++;
			$this->log->add('hf-order-csv-import', sprintf( __( '> Finished importing order %s', 'wf_order_import_export' ), $dry_run ? "" : $order->get_order_number() ) );


		$this->log->add( 'mark-order-csv-import',__( 'Finished processing orders.', 'wf_order_import_export' ) );
