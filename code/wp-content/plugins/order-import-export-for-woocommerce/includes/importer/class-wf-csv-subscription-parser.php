...
class WF_CSV_Subscription_Parser {
    ...
    public function parse_subscription_orders($parsed_data, $raw_headers, $merging, $record_offset) {
        ...
        if (empty($data['subscription_status'])) {
            $status = 'pending';
            $WF_CSV_Subscription_Order_Import->log->add(sprintf(__('No subscription status was specified. The subscription will be created with the status "pending". ', 'wf_order_import_export')));
        } else {
            ...
            switch ($date_type) {
                case 'end_date' :
                    if (!empty($dates_to_update['last_payment_date']) && strtotime($datetime) <= strtotime($dates_to_update['last_payment_date'])) {
                        $WF_CSV_Subscription_Order_Import->log->add(sprintf(__('The %s date must occur after the last payment date.', 'wf_order_import_export'), $date_type));
                    }

                    if (!empty($dates_to_update['next_payment_date']) && strtotime($datetime) <= strtotime($dates_to_update['next_payment_date'])) {
                        $WF_CSV_Subscription_Order_Import->log->add(sprintf(__('The %s date must occur after the next payment date.', 'wf_order_import_export'), $date_type));
                    }
                case 'next_payment_date' :
                    if (!empty($dates_to_update['trial_end_date']) && strtotime($datetime) < strtotime($dates_to_update['trial_end_date'])) {
                        $WF_CSV_Subscription_Order_Import->log->add(sprintf(__('The %s date must occur after the trial end date.', 'wf_order_import_export'), $date_type));
                    }
                case 'trial_end_date' :
                    if (strtotime($datetime) <= strtotime($dates_to_update['start'])) {
                        $WF_CSV_Subscription_Order_Import->log->add(sprintf(__('The %s must occur after the start date.', 'wf_order_import_export'), $date_type));
                    }
            }
        }
