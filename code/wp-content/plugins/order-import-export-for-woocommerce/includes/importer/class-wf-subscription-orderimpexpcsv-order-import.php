...
// Remainder
    ...
    private function process_subscription_orders($data) {
        ...
        $this->log->add(__('Process start.', 'wf_order_import_export'));
        ...
        if (is_wp_error($user_id)) {
            $this->log->add(sprintf(__($user_id->get_error_message(), 'wf_order_import_export')));
            ...
            $this->log->add(sprintf(__('An error occurred with the customer information provided.', 'wf_order_import_export')));
        ...
        if (!$dry_run) {
            ...
            if (!$merging && $is_order_exist) {
                ...
                $this->log->add('csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'wf_order_import_export'), esc_html($data['subscription_id'])), true);
                unset($data);
                return;
            } else {
                if ($is_order_exist) {
                    ...
                    if (is_wp_error($subscription)) {
                        ...
                        $this->log->add('csv-import', sprintf(__($subscription->get_error_message(), 'wf_order_import_export'), esc_html($data['subscription_id'])), true);
                        unset($data);
                        return;
            ...
            if ($data['manualy_set']) {
                ...
                $this->log->add('csv-import', sprintf(__($warning, 'wf_order_import_export'), $data['subscription_id']), true);
                //log warning
            }
        ...
        $this->log->add('csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'wf_order_import_export'), esc_html($data['subscription_id'])), true);
        $this->imported++;
        $this->log->add(sprintf(__('> Finished importing order %s', 'wf_order_import_export'), $dry_run ? "" : $data['subscription_id'] ));
