	 ...
	 */
		...
		foreach ( $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}woocommerce_tax_rates" ) as $_row ) {
			...
			if ( $row <= $record_offset ) {
				$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> Row %s - skipped due to record offset.', 'wf_order_import_export' ), $row ) );
				unset($item);
                                return;
			}
			...
			if ( $order_number_formatted ) {
				...
				if ( $order_id ) {
					...
					$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Order %s already exists.', 'wf_order_import_export' ), $order_number_formatted ) );
					$skipped++;
					unset($item);
                                        return;
				}
			...
			if ( isset( $item['customer_id'] ) && $item['customer_id'] ) {
				...
				if ( is_int( $item['customer_id'] ) ) {
					...
					if ( ! $found_customer ) {
						...
						$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Cannot find customer with id %s.', 'wf_order_import_export' ),  $item['customer_id'] ) );
						$skipped++;
                                                unset($item);
                                                return;;
			...
			if ( ! empty( $item['status'] ) ) {
				...
				if ( ! $found_status ) {
					$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Unknown order status %s (%s).', 'wf_order_import_export' ),  $item['status'], implode( $available_statuses, ', ' ) ) );
					$skipped++;					
                                        unset($item);
                                        return;
				}
			...
			if ( ! empty( $item['date'] ) ) {
				if ( false === ( $item['date'] = strtotime( $item['date'] ) ) ) {
					// invalid date format
					$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Invalid date format %s.', 'wf_order_import_export' ),  $item['date'] ) );
					$skipped++;
                                        unset($item);
                                        return;
				}
...
//							// invalid item
//							$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Missing SKU, quantity or total for %s on row %s.', 'wf_order_import_export' ),  'line_item_' . $i, $row ) );
//							$skipped++;
//                                                      $i++;
...
//						}
						...
						if ( ! $allow_unknown_products && ! $product_id ) {
							// unknown product
							$WF_CSV_Order_Import->log->add('order-csv-import', sprintf( __( '> > Skipped. Unknown order item: %s.', 'wf_order_import_export' ),  $product_identifier ) );
							$skipped++;
                                                        $i++;
							continue;  // break outer loop
						}
